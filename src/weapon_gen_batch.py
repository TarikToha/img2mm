...
def save_output_image(image_data, out_file):
...
def run_weapon_model(input1, input2):
...
if __name__ == "__main__":
    ...
    for case_idx, row in tqdm(dataset.iterrows(), total=total_cases, desc='Inference'):
        capture_dir, capture_id, target_dir = row['Dataset'], row['Capture_ID'], row['Target_Dir']
        label_clothing, label_env = row['Label_clothing'], row['Label_env']
        start_frame, end_frame = row['Start_Frame'], row['End_Frame']

        prefix = f'{args.image_type}_{capture_id}'
        capture_sym = object_type[capture_dir]
        prompt = generate_weapon_prompt(label_clothing, label_env)

        for frame_id in range(start_frame, end_frame):
            image_in_file = f'{args.input_dir}{capture_sym}_{prefix}_{frame_id}.jpg'
            assert os.path.exists(image_in_file), 'input file not found'
        ...
        if len(input_prompts) > 32 or case_idx == total_cases - 1:
            output_images = run_weapon_model(input_images, input_prompts)
